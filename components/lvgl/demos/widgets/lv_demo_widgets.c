...
static void profile_create(lv_obj_t * parent)
{
    ...
    // 以panel1为父对象创建一个label对象用以显示LV_SYMBOL_ENVELOPE这个图标，并将其样式设置为style_icon
    ...
    /*Create a keyboard*/
    // 以当前屏幕为父对象创建一个keyboard对象，配置其为隐藏（事先创建好，需要用到时将它显示出来）
    ...
    // 为user_name这个对象注册一个回调函数ta_event_cb（也写在lv_demo_widgets.c文件中了），用于唤出键盘
    ...
    // 以panel2为父对象创建一个dropdown对象，用于选择
    ...
    // 为birthdate这个对象注册一个回调函数birthday_event_cb，用于弹出日历进行选择
    ...
    // SMALL显示屏的布局
         ...
         * parent对象包含三个子对象    --panel1
           ...
           这里先划分控件                               
        */
        // 对于列来说1%的空间可被使用，不过这里好像没什么影响（1-99随便选）
        static lv_coord_t grid_main_col_dsc[] = {LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
        // 将行划分为根据内容调整的3行（即分别对应panel1，panel2和panel3）
         ...
         * panel1包含如下对象     --avatar          Avatar
                                ...
                                --invite_btn      Button2
        */
        // 分为两列，第一列根据内容分配，第二列为空余空间的1%，不过这里好像还是没什么影响（1-99随便选）
        ...
        // LV_GRID_FR(10), LV_GRID_FR(1)，这样的话，左边一列就会比右边宽，界面会变得可以左右滑动
        // LV_GRID_FR(50), LV_GRID_FR(50)，这样的话，两列还是一样宽，界面也不可以左右滑动
        static lv_coord_t grid_2_col_dsc[] = {LV_GRID_FR(1), LV_GRID_FR(1), LV_GRID_TEMPLATE_LAST};
        static lv_coord_t grid_2_row_dsc[] = {
                LV_GRID_CONTENT,  /*Title*/
                5,                /*Separator*/
        ...
        // panel2和panel3用一样的布局？为啥不会有影响呢？
        // panel2只有一列，第二列的分配是没有用的
        lv_obj_set_grid_dsc_array(panel2, grid_2_col_dsc, grid_2_row_dsc);
        lv_obj_set_grid_dsc_array(panel3, grid_2_col_dsc, grid_2_row_dsc);

        // LV_GRID_ALIGN_STRETCH代表拉伸，将pos设置为0使panel1位于第0列，将span设置为1使涉及到的列数为1列
        // LV_GRID_ALIGN_CENTER代表居中对齐，将pos设置为0使panel1位于第0行，将span设置为1使涉及到的行数为1行
        lv_obj_set_grid_cell(panel1, LV_GRID_ALIGN_STRETCH, 0, 1, LV_GRID_ALIGN_CENTER, 0, 1);

        // 将dsc（即描述）对象的对齐方式设置为居中对齐
        lv_obj_set_style_text_align(dsc, LV_TEXT_ALIGN_CENTER, 0);

        // 分别将子对象装进cell(/grid?)中
        // avatar有2列？
        // lv_obj_set_grid_cell(对象， 横向对齐方式， 横向顺序， 从起始位置涉及到的列数， 纵向对齐方式， 纵向顺序， 从起始位置涉及到的行数)
        // 将avatar对象放在第0列，第0行，并设置为纵向（0-1：共涉及2列）居中对齐，横向居中（0-0，共涉及1列）对齐
        ...
        // 将email_icn对象放在第0列，第4行，横向居中对齐，纵向居中对齐，不过图标对齐方式好像没什么用（因为上面设置第1列的宽度是根据内容调整的（代码第514行））
...
static void analytics_create(lv_obj_t * parent)
{
    ...
    // 设置char1_cont对象的高度，LV_PCT(x)，这个x好像是像素高度
    ...
    // 给char1对象注册一个回调函数chart_event_cb
    lv_obj_add_event_cb(chart1, chart_event_cb, LV_EVENT_ALL, NULL);
    // 如果是SMALL显示屏，那么设置x轴3倍聚焦（12个月只显示4个月，要看其他的需要拖动）
